...
def system_prompt(location_title: str | None) -> str:
    ...
    return (
        ...
        "- The audit may already have drafted content of its own: replies to particular "
        "reviews, a business description. list_audit_suggestions is where those drafts "
        "are. When the user asks you to act on the audit's suggestions, read them and "
        "apply the drafts as written rather than composing your own wording. Write your "
        "own when the user asks for something the audit did not draft. A suggestion the "
        "audit cannot apply says so, and is for the user to act on, not for you to "
